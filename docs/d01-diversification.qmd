...
## Correlation and covariance {auto-animate=true}
...
plus
<br>
$$2 w_1 w_2 \times \rho \times\text{std}(r_1)\times\text{std}(r_2)$$
...
The slope of the regression line of $r_2$ on $r_1$ is

. . .

$$\beta = \frac{\text{cov}(r_1, r_2)}{\text{var}(r_1)} = \rho \times \frac{\text{std}(r_2)}{\text{std(r_1)}}$$
